...
I've been using [[https://github.com/ajeetdsouza/zoxide][Zoxide]] (similar to [[https://github.com/wting/autojump][Autojump]], [[https://github.com/rupa/z][Z]], etc.) but I never liked how it's stateful: it "remembers" the directories  you've visited and lets you jump back to them later, e.g.:

#+begin_src console
  $ z foo/bar/baz
  $ cd -
  $ z baz # takes you back to foo/bar/baz
#+end_src

Since I only use a small subset of its features anyway, I decided to write my own, stateless version:

#+begin_src bash
  # z           -> fuzzy-find a repo (via ghq)
  # z [...args] -> fuzzy-find a directory
  z() {
    local dir
    if [[ -z $1 ]]; then
      ...
      # Or, if you want non-interactive:
      # dir=$(fd --type d . | fzf --filter "$*" | head -1) || return
    fi
    cd "$dir"
  }
#+end_src

I kind of snuck it in there but I use [[https://github.com/x-motemen/ghq][ghq]] for repository management. I like it.

...
//
// Code copied from an Apache-2.0 licensed project.
// Original copyright: Copyright 2019-2023 Dean Scarff.
// Licensed under the Apache License, Version 2.0.
// Full license text: LICENSE
//
// This code is included in compliance with the Apache-2.0 license.
...
static void ParseMode(const int argc, const char *argv[],
  ...
  errno = 0;
  const unsigned long width = strtoul(argv[kArgvWidthIndex], NULL, 10);
  if (errno != 0) {
    fprintf(stderr, "Error parsing width \"%s\": %s\n", argv[kArgvWidthIndex],
            strerror(errno));
    errno = 0;
    parsed_args->option = kOptionInvalidMode;
  }

  const unsigned long height = strtoul(argv[kArgvHeightIndex], NULL, 10);
  if (errno != 0) {
    fprintf(stderr, "Error parsing height \"%s\": %s\n", argv[kArgvHeightIndex],
            strerror(errno));
    errno = 0;
    parsed_args->option = kOptionInvalidMode;
  }
...
static void ShowUsage(void) { puts(kUsage); }
...
static void PrintMode(CGDisplayModeRef mode) {
  ...
  const bool usable_for_desktop = CGDisplayModeIsUsableForDesktopGUI(mode);
  printf("%zu x %zu @%.1fHz%s%s", width, height, refresh_rate,
         usable_for_desktop ? "" : " !", native ? " N" : "");
}
...
static CFDictionaryRef CreateDisplayModeOptions(CGDirectDisplayID display) {
  ...
  return CFDictionaryCreate(
      /* allocator= */ NULL, (const void **)option_keys,
...
static CGDisplayModeRef GetModeMatching(const struct ParsedArgs *parsed_args,
  ...
  for (CFIndex i = 0; i < count; ++i) {
    ...
    const size_t height = CGDisplayModeGetHeight(mode);
    const double refresh_rate = CGDisplayModeGetRefreshRate(mode);
    if (width == parsed_args->width && height == parsed_args->height &&
...
static int ConfigureMode(const struct ParsedArgs *parsed_args) {
  ...
  if (NULL == mode) {
    if (parsed_args->refresh_rate == 0.0) {
      ...
      fprintf(stderr,
              "Could not find a mode for resolution %lux%lu"
              " @%.1f\n",
              parsed_args->width, parsed_args->height,
